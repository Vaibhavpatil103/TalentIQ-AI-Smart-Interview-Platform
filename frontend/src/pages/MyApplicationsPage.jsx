      ...
      >
        ...
        <div className="max-w-5xl mx-auto px-6 py-8">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest text-[var(--dark-accent)] font-semibold mb-2">My Applications</p>
              <h1 className="text-2xl font-bold text-[var(--dark-text)]">Application Tracker</h1>
              <p className="text-[var(--dark-text-secondary)] text-sm mt-1">{applications.length} total applications</p>
            </div>
            <Link
              to="/jobs"
              className="bg-[var(--dark-accent)] hover:bg-[var(--dark-accent-hover)] text-white rounded-lg px-4 py-2 text-sm font-semibold transition-colors flex items-center gap-2"
            >
        ...
        <div className="max-w-5xl mx-auto px-6 mb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              { label: "Interviews", value: stats.interviews, color: "var(--color-purple)" },
              { label: "Offers", value: stats.offers, color: "var(--color-success)" },
            ].map(({ label, value, color }) => (
              <div key={label} className="bg-[var(--dark-card)] border border-[var(--dark-border)] rounded-xl p-4" style={{ borderTop: `2px solid ${color}` }}>
                <p className="text-2xl font-bold" style={{ color }}>{value}</p>
                <p className="text-xs text-[var(--dark-text-secondary)] mt-1">{label}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              <button
                ...
                className={`text-xs px-4 py-1.5 rounded-full font-medium border transition-colors ${
                  filterTab === f.key
                    ...
                    : "bg-transparent text-[var(--dark-text-secondary)] border-[var(--dark-border)] hover:text-[var(--dark-text)]"
                }`}
              >
                {f.label}
              </button>
        ...
        <div className="max-w-5xl mx-auto px-6 pb-16">
          {isLoading ? (
            <div className="space-y-3">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="bg-[var(--dark-card)] border border-[var(--dark-border)] rounded-xl h-24 animate-pulse" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div className="bg-[var(--dark-card)] border-2 border-dashed border-[var(--dark-border)] rounded-xl p-16 text-center">
              <InboxIcon className="size-12 text-[var(--dark-border)] mx-auto mb-4" />
              <p className="text-[var(--dark-text)] font-semibold">No applications yet</p>
              <Link to="/jobs" className="text-[var(--dark-accent)] text-sm hover:underline mt-3 block">Browse Jobs →</Link>
            </div>
          ) : (
            filtered.map((app) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-[var(--dark-card)] border border-[var(--dark-border)] rounded-xl p-5 mb-3 transition-all"
                  style={{
                    borderLeft: LEFT_BORDER[app.status] || "2px solid var(--dark-border)",
                ...
                >
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-lg bg-[var(--dark-elevated)] border border-[var(--dark-border)] flex items-center justify-center text-sm font-bold text-[var(--dark-accent)]">
                        {(job?.company || "?")[0].toUpperCase()}
                      </div>
                      <Link to={`/jobs/${jobId}`} className="font-semibold text-[var(--dark-text)] mt-3 block hover:text-[var(--dark-accent)] transition-colors">
                        {job?.title || "Unknown Position"}
                      </Link>
                      <div className="flex items-center gap-3 text-sm text-[var(--dark-text-secondary)] mt-1">
                    ...
                    <div className="flex flex-col items-end gap-2 flex-shrink-0 ml-3">
                      ...
                      <span className="text-[10px] text-[var(--dark-text-tertiary)]">
                  ...
                  {app.status === "interview_scheduled" && app.interviewScheduledAt && (
                    <div className="mt-3 pt-3 border-t border-[var(--dark-elevated)] flex items-center gap-2">
                      <CalendarIcon className="size-4 text-[#8250df]" />
                      <span className="text-xs text-[var(--dark-text-secondary)]">Interview scheduled:</span>
                  ...
                  {(app.status === "offer_sent" || app.status === "hired") && (
                    <div className="mt-3 pt-3 border-t border-[var(--dark-elevated)]">
                      <div className="rounded-lg p-3 flex items-center justify-between" style={{ background: "var(--color-success-bg)" }}>
                        <div className="flex items-center gap-2">
                          ...
                          {app.offeredSalary && (
                            <span className="text-[var(--dark-text-secondary)] text-sm">
                        ...
                        {app.status === "offer_sent" && (
                          <div className="flex gap-2">
                            <button
                              ...
                              className="bg-[var(--dark-accent)] text-white text-xs px-3 py-1 rounded-lg font-semibold hover:bg-[var(--dark-accent-hover)] transition-colors"
                            >
                              Accept
                            </button>
                            <button
                              onClick={() => respondToOffer({ id: app._id, response: "rejected" })}
                              className="border border-[var(--color-danger)] text-[var(--color-danger)] text-xs px-3 py-1 rounded-lg hover:bg-[var(--color-danger-bg)] transition-colors"
                            >
                              Decline
                            </button>
                          </div>
