...
function MiniStat({ icon: Icon, iconBg, topColor, label, value }) {
  return (
    <div
      className="bg-white border border-[#d0d7de] rounded-xl p-4 flex-1"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#d0d7de] flex-shrink-0">
          ...
          <button onClick={onClose} className="text-[#57606a] hover:text-[#1c2128]">
            <XIcon className="size-5" />
          </button>
        </div>
        <div className="p-6 space-y-5 overflow-y-auto flex-1">{children}</div>
        <div className="px-6 py-4 border-t border-[#d0d7de] flex gap-3 justify-end flex-shrink-0">
          {footer}
        </div>
      </motion.div>
    </div>
...
function CandidateDetailPanel({ app, jobs, onClose, onStatusChange, onSchedule, onOffer }) {
  ...
  return (
    <>
      ...
      >
        {/* HEADER */}
        <div className="px-6 py-4 border-b border-[#d0d7de] flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#0969da] text-white font-bold text-sm flex items-center justify-center">
              {initials}
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-2">
              {candidate?.email && (
                <div className="flex items-center gap-2">
                  <MailIcon className="size-4 text-[#57606a]" />
                  <span className="text-sm text-[#1c2128]">{candidate.email}</span>
                </div>
              )}
              {candidate?.githubUrl && (
                <div className="flex items-center gap-2">
                  <GithubIcon className="size-4 text-[#57606a]" />
                  <a href={candidate.githubUrl} target="_blank" rel="noreferrer" className="text-sm text-[#0969da] hover:underline">
                    {candidate.githubUrl}
                  </a>
                </div>
              )}
              {candidate?.resumeUrl && (
                <div className="flex items-center gap-2">
                  <FileTextIcon className="size-4 text-[#57606a]" />
                  <a href={candidate.resumeUrl} target="_blank" rel="noreferrer" className="text-sm text-[#0969da] hover:underline">
                    View Resume
                  </a>
                </div>
              )}
            ...
            {candidate?.techStack?.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {candidate.techStack.map((s) => (
                  <span key={s} className="bg-[#f6f8fa] text-[#57606a] border border-[#d0d7de] text-xs px-2 py-1 rounded-md">
                    {s}
                  </span>
                ))}
              </div>
            ) : (
              <p className="text-xs text-[#8c959f]">Not specified</p>
            )}
          </div>
          ...
          <div>
            ...
            {[
              ...
              <div key={label} className="flex justify-between py-2 border-b border-[#f6f8fa] last:border-0">
                ...
                <span className="text-sm font-medium text-[#1c2128]">{value}</span>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            {app.matchScore > 0 ? (
              <>
                ...
                {app.aiSummary && (
                  <p className="text-sm text-[#57606a] leading-relaxed mt-3">{app.aiSummary}</p>
                )}
                <div className="grid grid-cols-2 gap-3 mt-3">
                  {app.matchedSkills?.length > 0 && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-[#57606a] font-semibold mb-1.5">Matched</p>
                  ...
                  {app.missingSkills?.length > 0 && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-[#57606a] font-semibold mb-1.5">Missing</p>
                      <div className="flex flex-wrap gap-1">
              ...
              <p className="text-sm text-[#8c959f]">AI analysis not run yet</p>
            )}
          </div>
          ...
          {app.coverLetter && (
            <div>
              ...
              <div className="bg-[#f6f8fa] rounded-xl p-4">
                <p className="text-sm text-[#57606a] leading-relaxed whitespace-pre-wrap">{app.coverLetter}</p>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              className="bg-white border border-[#d0d7de] rounded-xl p-3 text-sm w-full min-h-[80px] resize-none outline-none focus:border-[#0969da] text-[#1c2128]"
              placeholder="Add private notes about this candidate..."
              value={localNotes}
              onChange={(e) => setLocalNotes(e.target.value)}
            />
            <button onClick={handleSaveNotes} className="text-xs text-[#0969da] hover:underline mt-2">
              Save Notes
            </button>
          </div>
        </div>
        ...
        {/* FOOTER */}
        <div className="px-6 py-4 border-t border-[#d0d7de] flex gap-3 flex-shrink-0 bg-white">
          <select
          ...
          >
            {ALL_STATUSES.map((s) => (
              <option key={s} value={s} className="bg-white text-[#1c2128]">
                {s.replace(/_/g, " ")}
              </option>
            ))}
          </select>
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-[#0969da] hover:bg-[#0550ae] text-white rounded-lg px-3 py-2 text-sm font-medium transition-colors disabled:opacity-50"
          >
  ...
  return (
    ...
    >
      <p className="text-sm text-[#57606a]">
        for <span className="font-semibold text-[#1c2128]">{candidateName}</span>
      </p>
      <div>
      ...
      <div>
        ...
        <p className="text-xs text-[#57606a] mt-1">An invite email will be sent automatically</p>
      </div>
    </ModalShell>
  );
}
...
function SendOfferModal({ app, onClose }) {
  ...
  return (
    ...
    >
      {/* Candidate card */}
      <div className="bg-[#f6f8fa] rounded-xl p-4 flex items-center gap-3">
        <div className="w-9 h-9 rounded-full bg-[#0969da] text-white text-sm font-bold flex items-center justify-center flex-shrink-0">
          {initials}
        </div>
        <div>
          <p className="font-medium text-sm text-[#1c2128]">{candidate?.name}</p>
          <p className="text-xs text-[#57606a]">for {job?.title || "this position"}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-[#57606a] mt-1">in {job?.currency || "USD"}</p>
      </div>
    ...
    <div className="min-h-screen bg-[#f6f8fa]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <MiniStat icon={UsersIcon} iconBg="#ddf4ff" topColor="#0969da" label="Total Applicants" value={appStats.total} />
      ...
      <div className="bg-white border-b border-[#d0d7de] px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-4 flex-wrap">
          <select
            className="bg-white border border-[#d0d7de] rounded-lg px-3 py-2 text-sm text-[#1c2128] outline-none focus:border-[#0969da]"
            value={selectedJob}
      ...
      >
        {/* Table header */}
        <div className="grid grid-cols-12 gap-4 px-4 py-3 text-xs uppercase tracking-wider text-[#57606a] border-b border-[#d0d7de] mb-2">
        ...
        {appsLoading || jobsLoading ? (
          <div className="space-y-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-white rounded-xl border border-[#d0d7de] h-16 animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="bg-white border-2 border-dashed border-[#d0d7de] rounded-xl p-16 text-center">
            ...
            <p className="font-semibold text-[#1c2128]">No candidates yet</p>
            <p className="text-sm text-[#57606a] mt-2">Applications will appear here once candidates apply</p>
          </div>
        ) : (
          filtered.map((app) => {
            const candidate = app.candidateObjectId;
            ...
            return (
              <motion.div
                ...
                className="grid grid-cols-12 gap-4 items-center bg-white border border-[#d0d7de]
                  rounded-xl px-4 py-4 mb-2 hover:border-[#0969da] hover:shadow-sm transition-all"
              >
                {/* Candidate (col 4) */}
                ...
                <div className="col-span-3">
                  <p className="text-sm text-[#1c2128] truncate">{job?.title || "—"}</p>
                ...
                <div className="col-span-1 flex justify-center">
                  {app.matchScore > 0 ? (
                    ...
                    <button
                      ...
                      disabled={aiPending}
                      className="text-[#0969da] text-xs hover:underline disabled:opacity-50 font-medium"
                    >
                ...
                <div className="col-span-2 flex items-center justify-end gap-2">
                  ...
                  <button
                    ...
                    className="border border-[#d0d7de] hover:border-[#0969da] text-[#57606a] text-xs px-2 py-1 rounded-md transition-colors"
                  >
                    Schedule
                  </button>
                </div>
